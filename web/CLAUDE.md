...
- **Two icon files, never `purpose: "any maskable"` on one.** The maskable drawing
  ...
  masked. `icon.svg` brings its own rounded shell; `icon-maskable.svg` is square
...
- **The icon never changes; the title does.** `titleFor` leads the document
  title with 🟠 or 🟢 by `summarySignal` over every worktree, and is the bare
  name otherwise. A title rather than a dot drawn on the favicon because it is
  the one surface that reaches the installed app's own window too -- its dock
  icon is the manifest's and cannot change, and it draws no favicon. There is no
  grey circle emoji, and grey would be the silence anyway. The circle leads in a
  browser tab and trails in the installed app, since Chrome shows an app
  window's title as-is only when it starts with the app's name and otherwise
  prefixes `Switchboard - ` to it.
...
:7999, and close the tab when you finish — a second viewer competes for terminal
geometry. Then:
